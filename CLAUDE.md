...
Note: `ZoomIt4Mac.xcodeproj` is generated by `xcodegen` from `project.yml` and is not committed (it's in `.gitignore`) — run `xcodegen` after pulling or editing `project.yml`.
